...
namespace FunkyCode.EventHandling
{
    ...
    {
        public static void GetCollisions(List<LightCollision2D> collisions, Light2D lightingSource)
        {
           var colliderList = LightCollider2D.ListEventReceivers;
            ...
            for(int i = 0; i < colliderList.Count; i++)
            {
                ...
                collision.points = new List<LightCollision2D.PointRelative>();
      
                for (int j = 0; j < polygon.points.Length; j++)
                {
                    Vector2 p = polygon.points[j] - (Vector2)lightingSource.transform.position;
                    ...
                    if (p.magnitude < lightingSource.size)
                    {
                        ...
                        {
                            collision.points.Add(new(j, p, polygon.points[j]));
                        }
                    }
                }
        ...
        public static LightCollision2D.PointRelative[] removePointsColliding = new LightCollision2D.PointRelative[100];
                    ...
                    for(int i = 0; i < polygons.Count; i++)
                    {
                        ...
                        for(int x = 0; x < pointsCount; x++)
                        {
                            ...
                            for(int c = 0; c < collisionCount; c++)
                            {
                                ...
                                {
                                    var point = col.points[p].lightRelative;

                                    if (Math2D.PointInPoly(point, testPolygon))
                                    {
                                        removePointsColliding[removePointsCollidingCount] = col.points[p];
